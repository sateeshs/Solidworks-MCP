...
def load_season(season_file: str) -> SeasonKnowledge:
    ...
    path = (_SEASONS_DIR / season_file).resolve()
    if not str(path).startswith(str(_SEASONS_DIR.resolve())):
        raise ValueError(f"Invalid season file path: {season_file}")
    if not path.is_file():
...
# Each archetype is a pre-designed combination of mechanisms that forms
# a coherent competitive strategy. These are the 8 competitive archetypes.
...
def get_archetype_by_id(archetype_id: str) -> DesignVariant | None:
    ...
    for v in get_archetypes():
        if v.archetype_id == archetype_id:
            return v
    return None
