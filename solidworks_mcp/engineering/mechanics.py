...
def calc_drivetrain_speed(
    ...
    wheel_diameter_mm : float
        Wheel outer diameter in mm.
    external_ratio : float
        Additional gear reduction *after* the motor gearbox (≥ 1.0).
    ...
    if external_ratio < 1.0:
        raise ValueError(
            f"external_ratio must be >= 1.0 (got {external_ratio}). "
            "Values < 1 imply overdrive, which is physically invalid."
        )

    wheel_circumference_m = math.pi * wheel_diameter_mm / 1000.0

    # motor.free_speed_rpm is the output shaft speed (post internal gearbox).
    # Only external_ratio applies to compute wheel RPM.
    output_rpm = motor.free_speed_rpm / external_ratio
    free_speed_m_s = (output_rpm * wheel_circumference_m) / 60.0

    # Loaded speed is typically ~80% of free speed under normal driving
    loaded_speed_m_s = free_speed_m_s * 0.80
    ...
    notes: list[str] = []
    notes.append(f"Motor internal ratio: {motor.gear_ratio:.1f}:1")
    notes.append(f"External ratio: {external_ratio:.1f}:1")
...
def calc_gear_ratio_for_speed(
    ...
    # target_speed = (motor.free_speed_rpm / external_ratio) * circumference / 60
    # => external_ratio = motor.free_speed_rpm * circumference / (60 * target_speed)
    ...
    # Current at operating speed ≈ free_current + fraction of (stall - free)
    # At ~80% free speed the load current is roughly 20% of stall current
    current_at_speed = motor.free_current_a + 0.20 * (
...
def calc_arm_torque(
    ...
    arm_length_mm : float
        Distance from pivot to load in mm.
    load_kg : float
        Mass at the end of the arm in kg.
    external_ratio : float
        Additional gear reduction after motor gearbox.
    """
    if arm_length_mm <= 0:
    ...
    # Usable torque is ~50% of stall (motor heats up and draws max current at stall)
    usable_torque = available_torque * 0.50
    ...
    max_load = usable_torque / arm_length_cm if arm_length_cm > 0 else 0.0
...
def calc_elevator_force(
    ...
    available_torque = motor.stall_torque_kg_cm * external_ratio * 0.50
    ...
    max_load = available_torque / spool_radius_cm if spool_radius_cm > 0 else 0.0
...
def calc_center_of_gravity(
    ...
    footprint_mm : (width, depth)
        Robot footprint for checking CG containment.
    """
    if not components:
    ...
    center_y = footprint_mm[1] / 2.0

    # Footprint check is on the horizontal plane: x (width) and y (depth).
    # z is vertical (height) and irrelevant for CG containment.
    offset = math.sqrt((cx - center_x) ** 2 + (cy - center_y) ** 2)
    within = (0 <= cx <= footprint_mm[0]) and (0 <= cy <= footprint_mm[1])
