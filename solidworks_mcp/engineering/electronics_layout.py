...
def _place_control_hub(
    ...
    # Hub dimensions (default REV Control Hub)
    hub_dims = hub.dimensions_mm if hub else (101.6, 63.5, 25.4)

    # Place at center, low height (on baseplate or first shelf)
...
def _place_expansion_hub(
    ...
    # Offset slightly from center to sit beside the control hub
...
def check_wire_clearances(
    ...
    motor_components = {"control_hub", "expansion_hub"}
    signal_components = set()

    for p in placements:
        if p.component not in motor_components:
            signal_components.add(p.component)

    for hub_p in placements:
        if hub_p.component not in motor_components:
            continue
        for sig_p in placements:
            if sig_p.component in motor_components:
                continue
            dist = math.sqrt(
                sum(
                    (a - b) ** 2
