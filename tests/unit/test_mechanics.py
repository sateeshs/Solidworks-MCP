...
from solidworks_mcp.engineering.models import MotorSpec, DrivetrainAnalysis, ArmAnalysis, CenterOfGravity, GearRatioResult
from solidworks_mcp.engineering.mechanics import (
    calc_arm_torque,
...
class TestCalcDrivetrainSpeed:
    ...
    def test_notes_include_ratio_info(self, motor_19_2: MotorSpec) -> None:
        result = calc_drivetrain_speed(motor_19_2, wheel_diameter_mm=96.0, external_ratio=2.0)
        assert any("ratio" in n.lower() for n in result.notes)
...
class TestCalcCenterOfGravity:
    ...
    def test_offset_from_center(self) -> None:
        # Component at center of footprint (x=228.5, y=228.5) → offset 0
        # y is depth (horizontal), z is height (vertical, ignored for CG footprint)
        components = [(228.5, 228.5, 50.0, 1000.0)]
        result = calc_center_of_gravity(components, footprint_mm=(457.0, 457.0))
